...
def mlp():
    model = Sequential()
    model.add(Dense(30, input_dim=1125, kernel_initializer='normal', activation='sigmoid'))
    model.add(Dense(20, input_dim=500, kernel_initializer='normal', activation='sigmoid'))
...
def main(plot=True):
    ...
    history = model.fit(x_train, y_train, batch_size=batch_size, verbose=1, epochs=35, validation_data=(x_test, y_test))
